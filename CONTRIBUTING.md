# Contributing to get-policy-done

Thank you for your interest in contributing to **get-policy-done** — an AI copilot for policy analysis and impact assessment.
...
git clone https://github.com/JesseRWeigel/get-policy-done.git
cd get-policy-done
...
Protocols live in `src/gpad/specs/references/protocols/`. Each file encodes a
...
1. Create a Markdown file in `src/gpad/specs/references/protocols/`, e.g.
   `regulatory-impact-analysis-protocols.md`.
2. Use YAML frontmatter to declare metadata:

   ```markdown
   ---
   domain: regulatory impact analysis
   version: "0.1"
   ---
...
Agents are Markdown files with YAML frontmatter in `src/gpad/agents/`.

Current agents:
  - `gpad-executor.md`
  - `gpad-modeler.md`
  - `gpad-paper-writer.md`
  - `gpad-planner.md`
  - `gpad-referee.md`
  - `gpad-researcher.md`
  - `gpad-verifier.md`
...
1. Create `src/gpad/agents/gpad-stakeholder-analyst.md`.
2. Include the required YAML frontmatter:

   ```markdown
   ---
   name: gpad-stakeholder-analyst
   ...
   <role>
   You are the **GPAD Stakeholder Analyst** — ...
   </role>
   ```
...
Verification predicates live in `src/gpad/core/kernel.py`. Each check is a
function that inspects evidence and returns a `CheckResult`.

To add a new check:

1. Open `src/gpad/core/kernel.py`.
2. Add a predicate function:

   ```python
   def check_stakeholder_coverage(evidence: dict[str, Any]) -> CheckResult:
       """Checks that all affected stakeholder groups are identified and analyzed."""
       # Inspect evidence...
       passed = ...  # your logic
       return CheckResult(
...
1. Create a Python module under `src/gpad/mcp/`, e.g.
   `src/gpad/mcp/stakeholder-analyst_server.py`.
2. Implement the server using the MCP SDK:

   ```python
   from mcp.server import Server

   server = Server("gpad-stakeholder-analyst")
...
get-policy-done follows the **Get-Paper-Done (GPD)** architecture — a shared framework
